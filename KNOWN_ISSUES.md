...
## Gameplay
1. **Set-piece control flow still needs tactical polish** - Throw-ins/corner-kicks/goal-kicks now apply dedicated restart shapes with baseline defender spacing, simple man-marking assignment, adaptive defender tracking, short-restart variants, and restart-specific defensive behavior modes (hold/contain/press); richer AI nuance and more variant depth are still pending.
2. **Penalty sequence is still prototype-grade** - Dedicated player-controlled penalty flow now includes basic keeper-save + rebound continuation, shot aim/power influence, keeper read/anticipation logic, and timed keeper commitment windows, but animation depth and advanced keeper behavior still need expansion.
